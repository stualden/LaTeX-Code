...
\ArticleTitle{1. Compound Interest Basics}
...
of time, as a percentage of the amount at the \emph{beginning} of the period.

The period is sometimes called an interest \textbf{conversion period}. It's called \emph{compound}
interest because each period's interest is based on \emph{all} past amounts including past interest.  So
...
\vspace{1em}
Actuaries use $v$ as shorthand for $\frac{1}{1+i}$ because it's used so often.
...
\begin{itemize}
  ...
  \item For anything beyond the simplest problems, drawing a timeline (even if just ``in your head'')
        is \emph{essential}!  It's the only way to ensure that your equation is correct.
\end{itemize}
