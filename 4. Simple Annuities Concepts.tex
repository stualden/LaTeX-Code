...
\ArticleTitle{4. Simple Annuities}
...
spaced, \emph{and} the rate period coincides with the payment period, we call them ``simple'' annuities.

The mathematics needed to value them is \emph{exactly} the same as what we've already seen for compound
interest, but when there are many payments, it is usually easier to use a calculator or computer.  There
are some useful computational shortcuts, however, and that is the subject of this concept sheet.  We'll
...
When actuaries want to specify the \emph{interest rate} and the \emph{term} of the value of
a \$1 annuity in a single symbol, they use
  {\Large
    \[
      \ax{\angln i}
    \]
