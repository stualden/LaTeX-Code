...
\finmathtitle{2. Nominal Interest Rates}

\setlength{\parskip}{0.75em}

So far, we've  talked about interest rates that are compounded once per period, where the period is a year.

Since a year is a long time to wait, interest is very often compounded more frequently than annually.  For example,
a bank might quote an interest rate of ``6\% per year, compounded monthly.''  This means that the interest is
actually being compounded 12 times per year, at a rate of 0.5\% (6\%/12) per month.

The 6\% here is often called the \textbf{nominal} annual interest rate, because it is 6\% ``in name only.'' The
0.5\% is called the \textbf{effective} monthly interest rate, because it's the \underline{actual} rate that is being
applied to the balance each month. The nomenclature isn't great, but it's used all the time, so we need to get
comfortable with it.

The effective annual rate (sometimes called EAR) is the rate that would generate the same amount of interest if it were
compounded once per year.  In our 6\% monthly example, the EAR would be the rate $j$ that satisfies the equation
\[
  (1 + j) = (1 + 0.005)^{12}
\]

Solving for j we get 0.061678, or about 6.2\%.  Note that this effective annual rate is \underline{higher} than the
...
Monthly is perhaps the most common compounding frequency (conversion period), but you will see many others, including quarterly (four times a year) and
daily.

Although theoretical, it is very useful to think about \underline{continuous} compounding, where the conversion
period shrinks down to nothing.  At that point, we have the pure exponential growth you may recall from "PERT"
problems in Algebra II.  In calculus, you'll see the amazing connection, where
...
\item When doing financial math exercises, always start by identifying the \underline{period} and
the corresponding effective \underline{rate per period}.
\item For a given nominal rate, the more frequent the compounding, the higher the effect annual rate.
