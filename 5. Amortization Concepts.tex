...
total payment, the interest portion, and the principal portion), and the outstanding principal
immediately after that payment has been made.  At the beginning, principal is just $ A $ (present value
of the annuity), and at the end, principal goes to \$0.
...
the loan, it can be calculated using the standard annuity formulas, either \emph{prospectively} or
\emph{retrospectively}.  Here, if
...
\begin{description}[align=right,labelwidth=3.5cm,leftmargin=!,font=\bfseries]
  ...
  \item[$\textbf{R} \cdot \mathbf{\sx{\angl{k} i}}$] Accumulated value of $k$ payments thus far
  \item[$\textbf{R} \cdot \mathbf{\ax{\angl{n-k} i}}$] Present value of the $(n-k)$ remaining payments
\end{description}
...
\begin{align*}
  P & = A \cdot (1+i)^k - R \cdot \sx{ \angl{k} i} \\
\end{align*}
